...
public class CountTreeNodeTests
{
    ...
    {
        var wrapping1 = new CountTreeNode<Edge, Node, Path, Builder>(
            new Node(12) { Children = new Dictionary<Edge, Node> { [new(2)] = new(20) } });
        var wrapping2 = new CountTreeNode<Edge, Node, Path, Builder>(
            new Node(12) { Children = new Dictionary<Edge, Node> { [new(2)] = new(20) } });
        Assert.AreEqual(wrapping1, wrapping2);

        var wrapping3 = new CountTreeNode<Edge, Node, Path, Builder>(
            new Node(13) { Children = new Dictionary<Edge, Node> { [new(2)] = new(20) } });
        Assert.AreNotEqual(wrapping1, wrapping3);

        var wrapping4 = new CountTreeNode<Edge, Node, Path, Builder>(
    ...
    public void WrappedNode_IsPreserved()
    {
        ...
        var wrapping = new CountTreeNode<Edge, Node, Path, Builder>(wrapped);
    ...
    public void DescendantsCount_OfLeafIsZero()
    {
        ...
        var leafWrapping = new CountTreeNode<Edge, Node, Path, Builder>(leafWrapped);
