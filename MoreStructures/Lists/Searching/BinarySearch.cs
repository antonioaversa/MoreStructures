...
/// 
/// <summary>
/// An object able to search in logarithmic time for elements in direct random access structures, such as lists and 
...
public class BinarySearch : ISearch
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// The algorithm split in half the search space at every iteration, reducing it exponentially to a single element
    /// or to an empty set.
    /// <br/>
    /// Time Complexity = O(log(n)), Space Complexity = O(1), where n = number of elements between 
    ...
    public int First<T>(
        ...
        Search(source, element, comparer, fromIndex, toIndex, true);
    ...
    public int Last<T>(
        IEnumerable<T> source, T element, IComparer<T>? comparer = null, int? fromIndex = null, int? toIndex = null) =>
        Search(source, element, comparer, fromIndex, toIndex, false);
    ...
    public (int first, int last) Interval<T>(
        IEnumerable<T> source, T element, IComparer<T>? comparer = null, int? fromIndex = null, int? toIndex = null)
    {
        var first = First(source, element, comparer, fromIndex, toIndex);
        if (first < 0)
            return (-1, -1);

        var last = Last(source, element, comparer, first, toIndex);
        return (first, last);
    }

    /// <inheritdoc/>
    /// <remarks>
    /// The algorithm first performs a binary search to find the index i of the 1st element. Then it checks whether 
    /// the n-th occurrence of the element exists at index i + n, taking advantage of the fact that 
